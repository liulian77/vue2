...
export default {
  ...
  mounted() {
    if(['edit','details'].indexOf(this.formMixinStatus) !== -1) {
  ...
  methods: {
    ...
    async formMixin_up(params) {
      try {
        ...
        // if(error !== 'form invalidate') {
        //   Message.warning('修改失败,请稍候再试!');
        // }
    ...
    async formMixin_add(params) {
      try {
        ...
        await this.formMixin_api_add(params || fp.omit(['_id'], this.formMixin_form));
        // 触发新增成功事件
        this.$emit('on-form-mixin-confirm');
        this.formMixin_emitTableRefresh();
      } catch (error) {
    ...
    formMixin_handleSubmitBtnClick() {
      if(this.formMixinStatus === 'edit') {
        this.formMixin_up();
      }else if(this.formMixinStatus === 'add') {
        this.formMixin_add();
      }
    },
    // 取消按钮点击事件
